...
impl ModelComponents {
    ...
    pub fn get_texture_coordinates(&self) -> Result<&TextureCoordinates, ErrBox> {
        self.texture_coordinates
            .as_ref()
            .ok_or("Can't get model texture coordinates".into())
    }

    pub fn get_vertices(&self) -> Result<&Vertices, ErrBox> {
        self.vertices
            .as_ref()
            .ok_or("Can't get model vertices".into())
    }

    pub fn get_texture_mapping(&self) -> Result<&PointsMapping, ErrBox> {
        self.texture_mapping
            .as_ref()
            .ok_or("Can't get texture points mapping".into())
    }
...
impl ModelData {
    pub fn create(
        vertices: Vertices,
        faces: Faces,
        texture_mapping: Option<PointsMapping>,
    ) -> Self {
...
pub fn make_data_source_opts(
    nodata: Option<HeightInt>,
    sea_level: Option<HeightInt>,
...
pub fn load_tile_data<'a>(
    data_source_path: &Path,
    data_source_name: &DataSourceName,
    opts: &'a dyn DataSourceOpts,
    tile_id: &'a TileID,
) -> Result<Option<impl TileData<'a>>, String> {
    match data_source_name {
...
pub trait Model<'a> {
    ...
    fn create_vertices_tiles<'b>(
        opts: &'b impl DataSourceOpts,
        vertices: &'b Vertices,
    ) -> VerticesToTilesMapping<'b> {
        ...
        for (k, geo_point) in vertices {
            ...
            match vertices_tiles.get_mut(&tile_id) {
                ...
                None => {
                    vertices_tiles.insert(tile_id, vec![(*k, geo_point)]);
                }
            }
        }
        vertices_tiles
    ...
    fn create(model_type: ModelType, settings: &'a Settings) -> Result<Self, ErrBox>
    where
        Self: Sized,
    {
        ...
        let ModelData(vertices, faces, texture_mapping) =
            Self::create_modeldata(model_size, spacing);
        let vertices_tiles = Self::create_vertices_tiles(&opts, &vertices);

        let texture_coordinates = match model_type {
            ModelType::Texture => Some(Self::create_texture_coordinates(model_size)),
        ...
        let color_mapping =
            ColorMapping::create(Path::new(&color_profile_file)).map_err(|err| {
                format!(
                    "Can't create color mapping from file '{}': {}",
                    &color_profile_file, err
                )
            })?;

        // Fill 'colors' with default color
        // The default color is the color of sea level
        let default_color = color_mapping
            .get_color(opts.get_sea_level())
            .map_err(|err| {
                format!(
                    "Can't get default color from color profile file '{}': {}",
                    &color_profile_file, err
                )
            })?;
        ...
        let mutex = Mutex::new(MutexStruct {
            heights: heights,
            colors: colors,
        });
                ...
                scope.spawn(|| {
                    while let Some(tile_id) = TileID::next(tiles_limit) {
                        ...
                        match vertices_tiles.get(&tile_id) {
                            Some(tile_vertices) => Self::calc_tile(
                                model_type,
                                settings,
                                data_source_name,
                                &opts,
