...
impl<'a> Model<'a> for Obj<'a> {
    ...
    fn create_modeldata(model_size: GeoPointIndex, j_spacing: Coord) -> ModelData {
        ...
        let mut faces: Faces =
            Vec::with_capacity(4 * (gnn as usize) * (gnn as usize) + 2 * (gnn as usize) + 1);
    ...
    fn create_texture_coordinates(model_size: GeoPointIndex) -> TextureCoordinates {
        ...
        for j in 1..gnn {
            let i_len = 4 * (gnn - j);
            let u_spacing = if i_len != 0 {
                1.0 / (i_len as TextureCoordinate)
            } else {
                0.0
            };
            for i in 0..=i_len {
                ...
                texture_coordinates.insert(
                    point_index,
                    (
                        u_spacing * i as TextureCoordinate,
                        0.5 + v_spacing * j as TextureCoordinate,
                    ),
                );
                point_index += 1;
                // south: even indices
        ...
        for i in 0..=0 {
            ...
            texture_coordinates.insert(
                point_index,
                (u_spacing * i as TextureCoordinate, 1.0 as TextureCoordinate),
            );
            point_index += 1;
            // south: even indices
            texture_coordinates.insert(
                point_index,
                (u_spacing * i as TextureCoordinate, 0.0 as TextureCoordinate),
            );
            point_index += 1;
        }
        ...
        Self: Sized,
    {
        ...
        let color_precision = settings
            .get_parameter_num("color_precision", DEFAULT_COLOR_PRECISION)?
            as ColorPrecision;
    ...
    fn save(&self) -> Result<(), ErrBox> {
        ...
        let create_mtl = || -> Result<(), ErrBox> {
            let mut data = match &self.model_type {
                ModelType::Color if self.color_precision == 0 => String::with_capacity(
                    2 * 22
                        * (self.color_precision + 1) as usize
                        * (self.color_precision + 1) as usize,
                ),
            ...
            let mtl_path = match mtl_path_opt.to_str() {
                ...
                None => {
                    return Err(format!(
                        "Can't make mtl file with path {:?} and name {}",
                        output_path, &planet_name
                    )
                    .into());
                }
            };
            ...
            // header
            data.clear();
            let f_tmpl = File::open(&self.template_file_mtl).map_err(|err| {
                format!(
                    "Can't open mtl template file {:?}: {}",
                    self.template_file_mtl, err
                )
            })?;
            let mut br = BufReader::new(f_tmpl);
            br.read_to_string(&mut data).map_err(|err| {
                format!(
                    "Can't read mtl template file {:?}: {}",
                    self.template_file_mtl, err
                )
            })?;
                    ...
                    for r_k in 0..=self.color_precision {
                        ...
                        for g_k in 0..=self.color_precision {
                            for b_k in 0..=self.color_precision {
                                data.push_str(
                                    format!("newmtl c_{}_{}_{}\n", r_k, g_k, b_k).as_str(),
                                );
                                let rgb = make_rgb_color(interval, r_k, g_k, b_k);
                                data.push_str(format!("Kd {}\n\n", rgb).as_str());
                            }
                        }
                        f_mtl.write_all(data.as_bytes()).map_err(|err| {
                            format!("Can't write to mtl file {}: {}", &mtl_path, err)
                        })?;
                    }
                }
            };
        ...
        let create_obj = || -> Result<(), ErrBox> {
            ...
            let result_path = match result_path_opt.to_str() {
                ...
                None => {
                    return Err(format!(
                        "Can't make obj file with path {:?} and name {}",
                        output_path, &planet_name
                    )
                    .into());
                }
            ...
            // header
            data.clear();
            let f_tmpl = File::open(&self.template_file_obj).map_err(|err| {
                format!(
                    "Can't open obj template file {:?}: {}",
                    self.template_file_obj, err
                )
            })?;
            let mut br = BufReader::new(f_tmpl);
            br.read_to_string(&mut data).map_err(|err| {
                format!(
                    "Can't read obj template file {:?}: {}",
                    self.template_file_obj, err
                )
            })?;
            ...
            f_obj.write_all(data.as_bytes()).map_err(|err| {
                format!("Can't write header to obj file {}: {}", &result_path, err)
            })?;
                ...
                match &self.model_type {
                    ModelType::Color if self.color_precision == 0 => {
                        let rgb = self
                            ...
                            .get_colors()?
                            .get(i)
                            .ok_or(format!("Missed color for point {}", i))?;
                        data.push_str(format!("v {:.5} {:.5} {:.5} {}\n", x, y, z, rgb).as_str())
                    }
                    _ => data.push_str(format!("v {:.5} {:.5} {:.5}\n", x, y, z).as_str()), // XXX: FRACTION_LENGHT=5
                }
                vertex_count += 1;
                if i % WRITER_BUF_STRINGS == WRITER_BUF_STRINGS - 1 {
                    f_obj.write_all(data.as_bytes()).map_err(|err| {
                        format!(
                            "Can't write chunk of vertices to obj file {}: {}",
                            &result_path, err
                        )
                    })?;
                    data.clear();
            ...
            f_obj.write_all(data.as_bytes()).map_err(|err| {
                format!("Can't write vertices to obj file {}: {}", &result_path, err)
            })?;
            ...
            if let ModelType::Texture = &self.model_type {
                ...
                for (u, v) in texture_coordinates.iter() {
                    ...
                    if coord_count % WRITER_BUF_STRINGS == WRITER_BUF_STRINGS - 1 {
                        f_obj.write_all(data.as_bytes()).map_err(|err| {
                            format!(
                                "Can't write chunk of texture coordinates to obj file {}: {}",
                                &result_path, err
                            )
                        })?;
                        data.clear();
                ...
                f_obj.write_all(data.as_bytes()).map_err(|err| {
                    format!(
                        "Can't write texture coordinates to obj file {}: {}",
                        &result_path, err
                    )
                })?;
            };
            ...
            for (tvt0, tvt1, tvt2) in self.components.get_faces()?.iter() {
                let vt0 = match pmap.get(tvt0) {
                    ...
                    None => {
                        return Err(
                            format!("Point tv0={} isn't found in points mapping", tvt0).into()
                        );
                    }
                };
                let vt1 = match pmap.get(tvt1) {
                    ...
                    None => {
                        return Err(
                            format!("Point tv1={} isn't found in points mapping", tvt1).into()
                        );
                    }
                };
                let vt2 = match pmap.get(tvt2) {
                    ...
                    None => {
                        return Err(
                            format!("Point tv2={} isn't found in points mapping", tvt2).into()
                        );
                    }
                };

                match &self.model_type {
                    ModelType::Texture => data.push_str(
                        format!(
                            "f {}/{} {}/{} {}/{}\n",
                            vt0 + 1,
                            tvt0 + 1,
                    ...
                    ModelType::Color => {
                        let color = self
                            ...
                            .get_colors()?
                            .get(vt0)
                            .ok_or(format!("Missed color for vertex {}", vt0))?;
                ...
                if faces_count % WRITER_BUF_STRINGS == WRITER_BUF_STRINGS - 1 {
                    f_obj.write_all(data.as_bytes()).map_err(|err| {
                        format!(
                            "Can't write chunk of faces to obj file {}: {}",
                            &result_path, err
                        )
                    })?;
                    data.clear();
            ...
            f_obj.write_all(data.as_bytes()).map_err(|err| {
                format!("Can't write faces to obj file {}: {}", &result_path, err)
            })?;
